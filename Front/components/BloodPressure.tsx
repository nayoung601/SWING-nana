...
//   const chartData = {
//     labels: bloodPressure.map((bp) => bp.measureTitle), // 아침/점심/저녁
//     datasets: [
//       {
//         data: bloodPressure.map((bp) => bp.highpressure), // 수축기 혈압
//         color: () => `rgba(255, 99, 132, 1)`, // 색상
//         strokeWidth: 2,
//       },
//       {
//         data: bloodPressure.map((bp) => bp.lowpressure), // 이완기 혈압
//         color: () => `rgba(54, 162, 235, 1)`, // 색상
//         strokeWidth: 2,
//       },
//     ],
//   };
...
export default function BloodPressure({ selectedDate, userId }) {
  ...
  const chartData = {
    labels: bloodPressure.map((bp) => bp.measureTitle), // 예: 아침, 점심, 저녁
    datasets: [
      {
        data: bloodPressure.map((bp) => parseFloat(bp.highpressure)), // 수축기 혈압
        color: () => `rgba(255, 99, 132, 1)`, // 수축기 색상 (빨간색)
      },
      {
        data: bloodPressure.map((bp) => parseFloat(bp.lowpressure)), // 이완기 혈압
        color: () => `rgba(54, 162, 235, 1)`, // 이완기 색상 (파란색)
      },
    ],
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal isVisible={isModalVisible} onBackdropPress={toggleModal}>
        <View style={styles.modalContainer}>
          <Text style={styles.modalTitle}>혈압 막대 그래프</Text>
          <BarChart
            data={{
            ...
            chartConfig={{
              ...
              color: () => 'rgba(0, 0, 0, 1)', // 막대 색상 (검정색)
              labelColor: () => 'rgba(0, 0, 0, 1)', // 라벨 색상
              barPercentage: 0.5,
